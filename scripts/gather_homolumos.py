...
for path_props, path_mol, xyz_path in zip(snakemake.input["propsfiles"],
    ...
    # fuk_neighbours_fuk = [(0,0)]*3
    # fuk_neighbours_chrg = [(0,0)]*3
    fuk_neighbours_fuk = [0]*3
    fuk_neighbours_chrg = [0]*3
    nei_fuk = [nei.GetIdx() for nei in neighbours]
    for i, ind in enumerate(nei_fuk[:3]):
        fuk_neighbours_fuk[i] += props_fuk_neg[ind]
        fuk_neighbours_chrg[i] += charges[ind]

    fuk_neighbours_fuk.sort(reverse=True)
    fuk_neighbours_chrg.sort()
