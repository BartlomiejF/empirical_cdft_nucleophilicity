...
r2_score(data["N Params"], data["ND"])
r2_score(data["N Params"], [data["N Params"].mean()]*data.shape[0])
...
r2 = r2_score(data["N Params"], data["pysr"])
minmax = [data["pysr"].min(),data["pysr"].max()]
fig, ax = plt.subplots(figsize=(15,8))
sns.scatterplot(data, x="pysr", y="N Params", hue="Solvent", ax=ax)
rmse = np.sqrt(mean_squared_error(data["N Params"], data["pysr"]))
...
for axx, solv in zip(itertools.chain.from_iterable(ax), solvents):
    ...
    r2 = r2_score(plotdata["N Params"], plotdata["pysr"])
    rmse = np.sqrt(mean_squared_error(plotdata["N Params"], plotdata["pysr"]))
...
data["pysr_acetonitrile"] = -data["gam"]+data["homo"]+np.sqrt((np.sqrt(data["fuk_minus"])*(data["gam"]/(data["nei_1_chrg"]*(data["fuk_minus"]-0.013))+1.55*10**3)+data["sasa"]))
data["pysr_ch2cl2"] = data["homo"]*(data["nei_3_chrg"]+(data["chrg"]-data["fuk_minus"])*(data["homo"]+12)-np.sqrt((data["chrg"]+data["nei_1_chrg"]+0.265)**2))
...
for axx, solv in zip(itertools.chain.from_iterable(ax), solvents):
    ...
    axx.set(title=f"{solv}", xlabel=f"$N_{{emp}}[{solv}]$", ylabel="$N_M$")
    r2 = r2_score(plotdata["N Params"], plotdata[f"pysr_{solv}"])
    linr = linregress(plotdata[f"pysr_{solv}"], plotdata["N Params"])
    rmse = np.sqrt(mean_squared_error(plotdata["N Params"], plotdata[f"pysr_{solv}"]))
    axx.text(0.1,0.8, f"$R^2$={r2:.3f}\nRMSE={rmse:.3f}", transform=axx.transAxes)
    axx.plot(minmax, [x*linr.slope + linr.intercept for x  in minmax], ls=":", linewidth=3, alpha=0.7, c="k")
fig.tight_layout()
