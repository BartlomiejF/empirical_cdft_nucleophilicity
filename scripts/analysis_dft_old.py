import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
from sklearn.metrics import r2_score, mean_squared_error
from rdkit import Chem
import morfeus
import check_nucleo_sites
from scipy.stats import linregress
import numpy as np
from rdkit.Chem.Scaffolds import MurckoScaffold
import json

plt.rcParams["font.size"] = 25
plt.rcParams["legend.fontsize"] = 15
plt.rcParams["legend.title_fontsize"] = 15
plt.rcParams["axes.labelsize"] = 20


data = []
for fil in snakemake.input["jsons"]:
    with open(fil, "r") as f:
        jsn = json.load(f)
        cid = int(jsn["molecule"].split("/")[-1][:-4])
        data.append({
            "cid":    cid,
            "ng":     jsn["reactivity_indices"]["NG"],
            "nd":     jsn["reactivity_indices"]["ND"],
            "homo":   jsn["orbital_energies"]["homo_eV"],
            "lumo":   jsn["orbital_energies"]["lumo_eV"],
            "homo_1": jsn["orbital_energies"]["homo_1_eV"],
            "mol":    Chem.MolFromMolFile(f"output/pyscf/{cid}_opt.mol", sanitize=False, removeHs=False),
            "fukui_minus":  jsn["fukui_minus_hirshfeld"],
            "cm5":    jsn["cm5_N"],
        })

mayr = pd.read_csv(snakemake.input["mayr"], names=["cid", "smiles","N","solvent"])

rows = []
for dat in data:
    cid, mol, fuks, chrgs = dat["cid"], dat["mol"], dat["fukui_minus"], dat["cm5"]
    rank = [fuk*chrg for fuk, chrg in zip(fuks, chrgs)]

    atoms = mol.GetAtoms()

    sites, *_ = check_nucleo_sites.find_nucleophilic_sites(mol)

    if len(sites) == 0:
        sites = list(range(len(rank)))

    rank_sites = [rank[i] for i in sites]
    rank_ind = rank.index(min(rank_sites))

    neighbours = atoms[rank_ind].GetNeighbors()
    neighbours = [[nei.GetIdx(), nei.GetAtomicNum(), nei.GetNeighbors(), chrgs[nei.GetIdx()]] for nei in neighbours]
    for n in neighbours:
        n[2] = sum([a.GetAtomicNum() for a in n[2]]) - atoms[rank_ind].GetAtomicNum()

    fuk_neighbours_fuk = [0]*3
    fuk_neighbours_chrg = [0]*3
    neighbours.sort(reverse=True, key=lambda x: (x[1], x[2], -x[3]))

    for i, ind in enumerate(neighbours[:3]):
        fuk_neighbours_fuk[i] += fuks[ind[0]]
        fuk_neighbours_chrg[i] += chrgs[ind[0]]

    # fuk_neighbours_fuk.sort(reverse=True, key=lambda x: (x[1], x[2], x[0]))
    # fuk_neighbours_chrg.sort(reverse=True, key=lambda x: (x[1], x[2], x[0]))
    
    # print(cid)
    elems, coords = morfeus.read_geometry(f"output/pyscf/{cid}_opt.xyz")
    sasa = morfeus.SASA(elems, coords)
    sasa_fuk_max = sasa.atom_areas[rank_ind+1]

    row = [cid]
    row.append(dat["ng"])
    row.append(dat["nd"])
    row.append(dat["homo"])
    row.append(dat["lumo"])
    row.append(dat["homo_1"])
    row.append(fuks[rank_ind])
    # row.append(str(props_fuk_pos[rank_ind]))
    row.append(chrgs[rank_ind])
    row.extend(fuk_neighbours_fuk)
    row.extend(fuk_neighbours_chrg)
    row.append(sasa_fuk_max)
    rows.append(row)

rows = pd.DataFrame(rows, columns=["cid","ng", "nd","homo","lumo","homo_1", "fuk_minus", "fuk_plus" , "chrg",
                                "nei_1_fuk", "nei_2_fuk", "nei_3_fuk",
                                "nei_1_chrg", "nei_2_chrg", "nei_3_chrg",
                                "sasa"])

data = pd.merge(mayr,rows, on="cid")

data["solv"] = data["solvent"].replace({"water": 78.4,
                                        "ch2cl2": 8.9,
                                        "DMF": 36.7,
                                        "THF": 7.6,
                                        "acetonitrile": 35.7,
                                        "DMSO": 46.7,
                                        })

data["dipole_solv"] = data["solvent"].replace({
    "water": 2.6089
})
data["omega_solv"] = 0.7003
data["homo_1_solv"] = -11.2164

data["mu"] = 0.5*(data["lumo"] + data["homo"])
data["eta"] = data["lumo"] - data["homo"]
data["gam"] = data["lumo"] - 2* data["homo"] + data["homo_1"]
data["omega"] = data["mu"]**2 / (2*data["eta"])


fig, ax = plt.subplots(figsize=(15,8))
linr = linregress(data["ng"], data["N"])
minmax = [data["ng"].min(),data["ng"].max()]
r2 = r2_score(data["N"], data["ng"]*linr.slope + linr.intercept)
rmse = np.sqrt(mean_squared_error(data["N"], data["ng"]*linr.slope + linr.intercept))
sns.scatterplot(data, x="ng", y="N", ax=ax)
ax.plot(minmax, [x*linr.slope + linr.intercept for x  in minmax], ls=":", linewidth=3, alpha=0.7, c="k")
ax.text(0.1,0.8, f"$R^2$={r2:.3f}\nRMSE={rmse:.3f}", transform=ax.transAxes)
ax.set(xlabel="$N_G$", ylabel="$N_M$")
fig.tight_layout()
fig.savefig("imgs/ng_dft.pdf")


fig, ax = plt.subplots(figsize=(15,8))
linr = linregress(data["nd"], data["N"])
minmax = [data["nd"].min(),data["nd"].max()]
r2 = r2_score(data["N"], data["nd"]*linr.slope + linr.intercept)
rmse = np.sqrt(mean_squared_error(data["N"], data["nd"]*linr.slope + linr.intercept))
sns.scatterplot(data, x="nd", y="N", ax=ax)
ax.plot(minmax, [x*linr.slope + linr.intercept for x  in minmax], ls=":", linewidth=3, alpha=0.7, c="k")
ax.text(0.1,0.8, f"$R^2$={r2:.3f}\nRMSE={rmse:.3f}", transform=ax.transAxes)
ax.set(xlabel="$N_D$", ylabel="$N_M$")
fig.tight_layout()
fig.savefig("imgs/nd_dft.pdf")


data["pysr"] = 2.6089 + (2.6089*data["fuk_minus"] - data["chrg"]-2.69)**2 * (data["homo"]+(-9.2128)+-3.8691+30.5) + 13.9

fig, ax = plt.subplots( figsize=(15,8))
linr = linregress(data["pysr"], data["N"])
minmax = [data["pysr"].min(),data["pysr"].max()]
r2 = r2_score(data["N"], data["pysr"]*linr.slope + linr.intercept)
rmse = np.sqrt(mean_squared_error(data["N"], data["pysr"]*linr.slope + linr.intercept))
sns.scatterplot(data, x="pysr", y="N", ax=ax)
ax.plot(minmax, [x*linr.slope + linr.intercept for x  in minmax], ls=":", linewidth=3, alpha=0.7, c="k")
ax.text(0.1,0.3, f"$R^2$={r2:.3f}\nRMSE={rmse:.3f}", transform=ax.transAxes)
ax.set(xlabel="$N_{emp}$", ylabel="$N_M$")
fig.tight_layout()
fig.savefig("imgs/pysr_dft.pdf")


data["pysr_water"] = (0.968 - data["nei_1_chrg"])/(data["nei_1_fuk"]/(1.57-data["gam"])-8.55*data["nei_2_chrg"]+0.604) + 13.3

fig, ax = plt.subplots( figsize=(15,8))
linr = linregress(data["pysr_water"], data["N"])
minmax = [data["pysr_water"].min(),data["pysr_water"].max()]
r2 = r2_score(data["N"], data["pysr_water"]*linr.slope + linr.intercept)
rmse = np.sqrt(mean_squared_error(data["N"], data["pysr_water"]*linr.slope + linr.intercept))
sns.scatterplot(data, x="pysr_water", y="N", ax=ax)
ax.plot(minmax, [x*linr.slope + linr.intercept for x  in minmax], ls=":", linewidth=3, alpha=0.7, c="k")
ax.text(0.8,0.8, f"$R^2$={r2:.3f}\nRMSE={rmse:.3f}", transform=ax.transAxes)
ax.set(xlabel="$N_{emp}[water]$", ylabel="$N_M$")
fig.tight_layout()
fig.savefig("imgs/pysr_water_dft.pdf")


def group_by_scaffold(smiles_series, return_scaffold_smiles=False, scaffold_type='murcko'):
    """
    Group molecules by scaffolds.
    
    scaffold_type options:
    - 'murcko': Bemis-Murcko scaffolds (rings only)
    - 'murcko_generic': Generic Murcko (all atoms as carbon)
    - 'functional': Group by functional groups for non-ring molecules
    """
    scaffold_dict = {}
    scaffold_labels = []
    next_id = 0
    
    for smiles in smiles_series:
        try:
            mol = Chem.MolFromSmiles(smiles)
            
            if scaffold_type == 'murcko_generic':
                scaffold = MurckoScaffold.MakeScaffoldGeneric(
                    MurckoScaffold.GetScaffoldForMol(mol)
                )
                scaffold_smiles = Chem.MolToSmiles(scaffold)

            else:  # murcko
                scaffold = MurckoScaffold.GetScaffoldForMol(mol)
                scaffold_smiles = Chem.MolToSmiles(scaffold)
            
            # Fallback for empty scaffolds
            if not scaffold_smiles or scaffold_smiles == '' or scaffold_smiles == None:
                scaffold_smiles = 'acyclic'
            
            if return_scaffold_smiles:
                scaffold_labels.append(scaffold_smiles)
            else:
                if scaffold_smiles not in scaffold_dict:
                    scaffold_dict[scaffold_smiles] = next_id
                    next_id += 1
                scaffold_labels.append(scaffold_dict[scaffold_smiles])
        except:
            scaffold_labels.append(None)
    
    return pd.Series(scaffold_labels, index=smiles_series.index)

data["group_murcko"] = group_by_scaffold(data["smiles"], return_scaffold_smiles=True, scaffold_type="murcko")

def get_pysr_prediction(row):
    scaffold = row["group_murcko"]
    
    if scaffold == "acyclic":
        return np.log((-row["omega"] + row["solv"] + (-1.62*row["dipole_solv"]**2 + 1.62*row["omega_solv"])**2 * (row["homo"] - row["homo_1_solv"] - 0.889)**2)**2)
    elif scaffold == "c1ccccc1":
        return row["dipole_solv"] - row["gam"] + (row["dipole_solv"]**2 * row["nei_1_fuk"]**2 + 0.248*(-2.25*row["chrg"] + row["homo"]))**3 + 22.7
    elif scaffold == "c1ccncc1":
        # return -np.log((-4.18e3*row["nei_1_fuk"]**2 + np.exp(8.38*row["fuk_plus"]))**2) - 28.5 - 473/row["homo"]
        return np.nan
    elif scaffold == "c1ccc2[nH]ccc2c1":
        # return -row["fuk_plus"]*(row["sasa"] + 2.93/(row["mu"] + row["sasa"] + 1.04)) - 0.252*row["homo_1"]*row["mu"] + 31.1
        return np.nan
    elif scaffold == "C1CCNC1":
        return np.log(np.sqrt((0.0152*row["gam"]*(6.26 - row["sasa"]) + row["nei_3_chrg"])**2)) + 18.2 - 1/(row["mu"] + row["sasa"])
    elif scaffold == "c1ccc(P(c2ccccc2)c2ccccc2)cc1":
        return -9.97*row["gam"]*row["nei_1_chrg"] + 1/(row["chrg"] - 0.893 + 3.04/row["eta"]) + 293/row["omega"]
    else:
        return np.nan

data["pysr_prediction"] = data.apply(get_pysr_prediction, axis=1)

d = data.dropna()

fig, ax = plt.subplots( figsize=(15,8))
linr = linregress(d["pysr_prediction"], d["N"])
minmax = [d["pysr_prediction"].min(),d["pysr_prediction"].max()]
r2 = r2_score(d["N"], d["pysr_prediction"]*linr.slope + linr.intercept)
rmse = np.sqrt(mean_squared_error(d["N"], d["pysr_prediction"]*linr.slope + linr.intercept))
sns.scatterplot(d, x="pysr_prediction", y="N", ax=ax)
ax.plot(minmax, [x*linr.slope + linr.intercept for x  in minmax], ls=":", linewidth=3, alpha=0.7, c="k")
ax.text(0.5,0.3, f"$R^2$={r2:.3f}\nRMSE={rmse:.3f}", transform=ax.transAxes)
ax.set(xlabel="$N_{emp}<scaffold>$", ylabel="$N_M$")
fig.tight_layout()
fig.savefig("imgs/pysr_scaffold_dft.pdf")